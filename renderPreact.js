import { h, Component, render } from './vendor/preact-10-15-1.js';
...
function ListItem(props) {
    ...
    <div class="list-item ${props.read && "read"}">
        ...
        <input type="checkbox" class="read-checkbox" checked="${!!props.read}" onClick="${e => props.onReadToggle(props, e.target.checked)}"/>
...
function App(props) {
    ...
    return html`
    <h1>${props.heading}</h1>
    ${props.readingList.map(i => html`<${ListItem} ...${i} onDelete="${props.onDelete}" onReadToggle="${props.onReadToggle}" />`)}
    `;
}
...
export function renderPreact(heading, el, readingList, onDelete, onReadToggle) {
    ...
    render(html`
        <${App} heading="${heading}" readingList="${readingList}" onDelete="${onDelete}" onReadToggle="${onReadToggle}" />
    `, listContainer);
}